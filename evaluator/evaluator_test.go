...
func testIntegerObject(t *testing.T, obj object.Object, expected int64) bool {
	...
	if !ok {
		t.Errorf("object is not Integer. got=%T (%+v), want=%d", obj, obj, expected)
		return false
	}
...
func TestFunctionObject(t *testing.T) {
	...
	if fn.Parameters[0].String() != "x" {
		t.Fatalf("parameter has wrong name, expected='x', got=%q", fn.Parameters[0])
	}
	expectedBody := "(x + 2)"
...
func TestHashLiterals(t *testing.T) {
	...
	if len(result.Pairs) != len(expected) {
		t.Fatalf("Hash has wrong num of parts. got=%d", len(result.Pairs))
	}
